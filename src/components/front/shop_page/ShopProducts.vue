  ...
  <div class="shop-products">
    <v-container>
      <div class="search mt-6 d-flex gc-2 ">
        <v-text-field type="search" variant="outlined" v-model="search.name" label="Search For Specific Product" color="#274C5B" />
        ...
        <v-btn  color="#274C5B" text="search" class="elevation-0" :loading height="54px" min-width="115px" append-icon="mdi-magnify" @click="() => { page = 1; filterProducts(); }"/>
      </div>
      <v-row class="py-6" justify="center" v-if="!loading">
        <v-col cols="9" sm="6" md="4" lg="3" v-for="product in data"
          :key="product.id">
         <ProductItem
            :rating="4.5"
