<template>
  <v-row class="mt-8 cursor-default justify-center">
    <v-col
      cols="12"
      sm="10"
      :md="!authStore.isCustomer || !product.canRate ? '10' : 6"
    >
      <v-card elevation="0" v-if="product.feedbacks.length > 0">
        <v-card-title class="text-h6 title roboto"
          >Customer Reviews</v-card-title
        >
        ...
        <v-list lines="two" class="customer-comments has-scroll">
          <v-list-item
            ...
            :key="r.id"
            :class="[
              'py-4 feedback main-transition text-grey-lighten-3',
        ...
        <v-row class="text-center">
          <v-col cols="12">
            ...
            <p
              ...
              v-if="authStore.isCustomer && product.canRate"
            >
...
const submitRating = async () => {
  ...
  try {
    ...
    axiosClient
      .post(`/send-feedback/${props.product.slug}`, newRating.value)
      .then((response) => {
        ...
        if (existingIndex !== -1) {
          props.product.feedbacks.splice(
            existingIndex,
            1,
